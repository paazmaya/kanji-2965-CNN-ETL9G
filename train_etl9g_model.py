    ...
    def _depthwise_separable_conv(self, in_channels, out_channels, stride=1):
        ...
        return nn.Sequential(
            ...
            nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=stride, 
                     padding=1, groups=in_channels, bias=False),  # groups=in_channels makes it depthwise
            nn.BatchNorm2d(in_channels),  # Normalization for training stability
            nn.ReLU(inplace=True),        # Activation function
            
            # Pointwise convolution (channel mixing)
            nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False),  # 1x1 conv for channels
...
def create_balanced_loaders(X, y, batch_size, test_size=0.15, val_size=0.15):
    ...
    if min_samples < 2:
        print("⚠️  Warning: Some classes have only 1 sample. Using non-stratified splitting for small datasets.")
...
def main():
    ...
    parser.add_argument('--sample-limit', type=int, default=None, help='Limit samples for testing (e.g., 50000)')
    parser.add_argument('--class-limit', type=int, default=None, help='Limit to N most frequent classes for testing (e.g., 100)')
    ...
    if args.class_limit and args.class_limit < num_classes:
        print(f"Limiting dataset to {args.class_limit} most frequent classes for testing...")
