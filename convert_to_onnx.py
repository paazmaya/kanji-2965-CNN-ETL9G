...
class LightweightKanjiNet(OriginalLightweightKanjiNet):
    ...
    def __init__(self, num_classes: int, image_size: int = 64, pooling_type: str = 'adaptive_avg'):
        ...
        if pooling_type == 'adaptive_avg':
            ...
            self.global_pool = nn.AvgPool2d(kernel_size=4, stride=1, padding=0)  # Compatible: AveragePool in ONNX
            print(f"🔍 Using AvgPool2d(4) -> AveragePool in ONNX")
        elif pooling_type == 'fixed_max':
            self.global_pool = nn.MaxPool2d(kernel_size=4, stride=1, padding=0)  # Compatible: MaxPool in ONNX
...
def export_to_onnx(model_path, onnx_path, image_size, num_classes, pooling_type='adaptive_avg', target_backend='tract'):
    ...
    if target_backend == 'ort-tract':
        ...
        if pooling_type in ['adaptive_avg', 'adaptive_max']:
            ...
            print(f"⚠️  ORT-Tract: Overriding {original_pooling} -> {pooling_type} (GlobalAveragePool unsupported)")
        ...
        if pooling_type in ['adaptive_avg', 'adaptive_max']:
            original_pooling = pooling_type
            pooling_type = 'fixed_avg' if pooling_type == 'adaptive_avg' else 'fixed_max'
            print(f"⚠️  Strict mode: Overriding {original_pooling} -> {pooling_type} (Maximum compatibility)")
    ...
    try:
        torch.onnx.export(
            ...
            dynamic_axes={
                'input': {0: 'batch_size'},
                'output': {0: 'batch_size'}
            } if target_backend != 'strict' else None,  # Disable dynamic axes for ultra-conservative mode
            dynamo=use_dynamo,
            verbose=False
        )
        print(f"✅ Export successful using {'torch.export (dynamo)' if use_dynamo else 'TorchScript'} method")
        ...
        if use_dynamo:
            ...
            torch.onnx.export(
                ...
                dynamic_axes={
                    'input': {0: 'batch_size'},
                    'output': {0: 'batch_size'}
                } if target_backend != 'strict' else None,
                verbose=False
            )
...
def main():
    ...
    parser.add_argument('--model-path', type=str, default='best_kanji_model.pth',
                       help='Path to the trained PyTorch model')
    parser.add_argument('--onnx-path', type=str, default='kanji_etl9g_model.onnx',
                       help='Output path for ONNX model')
    parser.add_argument('--image-size', type=int, default=64,
                       help='Image size used in training')
    parser.add_argument('--num-classes', type=int, default=3036,
                       help='Number of classes')
    parser.add_argument('--pooling-type', type=str, default='adaptive_avg',
                       choices=['adaptive_avg', 'adaptive_max', 'fixed_avg', 'fixed_max'],
                       help='Pooling layer configuration')
    parser.add_argument('--target-backend', type=str, default='tract',
    ...
    if result:
        ...
        print(f"  - {args.onnx_path} ({Path(args.onnx_path).stat().st_size / (1024*1024):.1f} MB)")
